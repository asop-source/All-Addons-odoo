...
class doc(models.Model):
	...
	user_id			= fields.Many2one("res.users", string="Dari", required=True, states={"draft": [("readonly", False)]})
	to_user_ids 	= fields.One2many("eo.to_user","doc_id",string="Kepada", ondelete="cascade", required=True, states={"draft": [("readonly", False)]})
	cc_user_ids		= fields.One2many("eo.cc_user","doc_id",string="Tembusan", ondelete="cascade", states={"draft": [("readonly", False)]})
	date			= fields.Date(string="Tanggal", required=True, states={"draft": [("readonly", False)]})
	...
	# #########################################################################
	# def replace_tokens(self, id, context=None):
	# 	return 

	# #########################################################################
	# # insert history
	# #########################################################################
	# def insert_history(self, doc_id, name, context=None):
	...
	def action_draft(self):
		...
		# return self.write(ids,{'state':DOC_STATES[0][0]},context=context)
	...
	def action_open(self):
		...
		# return self.write(ids,{'state':DOC_STATES[1][0]},context=context)
	...
	def action_send(self):
		...
		# Doc = self.browse(ids, context=context)[0]
		...
		# self.message_post(ids , body=body, partner_ids=partner_ids, context=context)

		# self.insert_history(ids[0], 'Set to Unread')
		# return self.write(ids,{'state':DOC_STATES[2][0]},context=context)

	@api.multi
	def action_read(self,ids,context=None):
		self.state= DOC_STATES[3][0]
		# self.insert_history(ids[0], 'Set to Read')
		# return self.write(ids,{'state':DOC_STATES[3][0]},context=context)

	@api.depends
	def action_reply(self,ids,context=None):
		'''
		...
		# history 
		######################################################################
		self.insert_history(ids[0], 'Replied')
	...
	def action_forward(self,ids,context=None):
		'''
		redirect to eo.doc form view with prefilled values 
		from the old doc
		'''
		...
		# history 
		######################################################################
		self.insert_history(ids[0], 'Forwarded')
	...
	#########################################################################
	def onchange_doc_template(self, ids, doc_template_id ):
		# return harus berupa dict yang berisi key = 'value'
		# setiap value berupa dict yang berisi nilai dari field lain
		#   yang mau diupdate 
		doc_template = self.pool.get('eo.doc_template').browse(doc_template_id, context=None)
